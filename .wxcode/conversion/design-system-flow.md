# Design System Collection Flow

This document describes the design system collection flow during `/wxcode:new-project`.

## Overview

The design system collection phase captures visual design decisions to generate a DTCG (Design Token Community Group) file that serves as the **Source of Truth** for all UI generation.

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    /wxcode:new-project                                  │
├─────────────────────────────────────────────────────────────────────────┤
│  Phase 1: Project Context                                               │
│  Phase 2: Stack Selection                                               │
│  Phase 3: Database Schema                                               │
│  Phase 4: Design System Collection  ← THIS PHASE                        │
│  Phase 5: Project Foundation                                            │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## Output Files

After collection, the following files are generated:

```
project/
├── design/
│   ├── tokens.json           # DTCG format (source of truth)
│   ├── tailwind.config.ts    # Stack-specific (if Tailwind)
│   ├── variables.css         # Stack-specific (if CSS vanilla)
│   └── README.md             # Human-readable documentation
```

---

## Collection Methods

Three methods are available, presented in order of recommendation:

| Method | Best For | Accuracy | Speed |
|--------|----------|----------|-------|
| **URL** | Existing sites, competitors | High (exact values) | Fast |
| **Screenshots** | Mockups, mobile apps, offline sites | Medium (approximate) | Medium |
| **Questionnaire** | From scratch, specific requirements | High (user-defined) | Slow (~12 questions) |

---

## Step 1: Method Selection

Present the user with three options:

```
┌─────────────────────────────────────────────────────────────────────────┐
│  DESIGN SYSTEM CONFIGURATION                                           │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  To convert your project, I need to understand the target design.      │
│  How would you like to provide this information?                        │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  ⭐ OPTION 1: Reference URL (Recommended)                       │   │
│  │  ─────────────────────────────────────────────────────────────  │   │
│  │  Paste the URL of a site with the desired visual style.         │   │
│  │  I'll automatically extract: colors, fonts, spacing,            │   │
│  │  shadows, border-radius, and more.                              │   │
│  │                                                                 │   │
│  │  ✅ Most accurate (exact px, hex, font names)                   │   │
│  │  ✅ Fastest (fewer questions)                                   │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  📸 OPTION 2: Screenshots                                       │   │
│  │  ─────────────────────────────────────────────────────────────  │   │
│  │  Send 1 or more reference images.                               │   │
│  │  I'll analyze visually and extract what's possible.             │   │
│  │                                                                 │   │
│  │  ⚠️ Approximate values (review/adjust needed)                   │   │
│  │  ✅ Useful for mockups, mobile apps, offline sites              │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  📝 OPTION 3: Manual Questionnaire                              │   │
│  │  ─────────────────────────────────────────────────────────────  │   │
│  │  Answer questions about your preferences.                       │   │
│  │  Ideal if you know exactly what you want.                       │   │
│  │                                                                 │   │
│  │  ⚠️ Slower (~12-15 questions)                                   │   │
│  │  ✅ Full control over every decision                            │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

**For WinDev/WebDev conversions:** If the user wants to preserve the legacy visual style, they can provide the URL or screenshot of the current system.

---

## Step 2a: URL Extraction

### Input

```
┌─────────────────────────────────────────────────────────────────────────┐
│  URL EXTRACTION                                                         │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Paste the reference site URL:                                          │
│  [https://_______________________________________________]              │
│                                                                         │
│  💡 Tip: Can be the current company site, a competitor,                │
│     or any site with the desired visual style.                         │
│                                                                         │
│  [ ] Include dark mode (if available)                                   │
│  [ ] Analyze mobile version too                                         │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### Extraction Process

1. **Primary: WebFetch**
   - Fetch the URL content
   - Parse CSS for colors, fonts, spacing
   - Extract computed styles

2. **Fallback: MCP Playwright**
   - If WebFetch fails (CSP, JS-rendered, blocked)
   - Use Playwright to render and extract
   - Access computed styles via JavaScript

```python
# Extraction targets
extraction = {
    "colors": {
        "from_css_variables": True,      # --primary, --secondary, etc.
        "from_computed_styles": True,    # background, color, border
        "from_logo": True,               # Dominant colors
        "from_buttons": True,            # CTA colors
    },
    "typography": {
        "font_families": True,
        "font_sizes": True,
        "font_weights": True,
        "line_heights": True,
    },
    "spacing": {
        "detect_base": True,             # 4px or 8px system
        "padding_values": True,
        "margin_values": True,
        "gap_values": True,
    },
    "shadows": {
        "box_shadows": True,
        "text_shadows": False,           # Usually not needed
    },
    "borders": {
        "border_radius": True,
        "border_widths": True,
    }
}
```

### Confidence Levels

Extracted values are categorized by confidence:

| Level | Criteria | Action |
|-------|----------|--------|
| **High** | Found in CSS variables, logo, CTAs, repeated patterns | Auto-include |
| **Medium** | Found in secondary elements, less frequent | Ask user to confirm |
| **Low** | Ambiguous, could be incidental | Show but don't include by default |

### Preview and Confirmation

```
┌─────────────────────────────────────────────────────────────────────────┐
│  ✅ DESIGN SYSTEM EXTRACTED                                            │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Review and adjust if needed:                                           │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│  COLORS                                                    [Edit ✏️]   │
│  ──────────────────────────────────────────────────────────────────────│
│  High confidence (detected in logo, CTAs, main elements):              │
│                                                                         │
│  ● Primary      #635bff  ████  ← Detected in logo and buttons          │
│  ● Text         #0a2540  ████  ← Main text color                       │
│  ● Background   #f6f9fc  ████  ← Predominant background                │
│  ● Accent       #00d4ff  ████  ← Highlights and links                  │
│                                                                         │
│  Medium confidence (may be useful):                                     │
│  ○ #425466  ████   ○ #7a73ff  ████   ○ #1ea672  ████                   │
│                                                                         │
│  [ ] Include medium confidence colors in design system                  │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│  TYPOGRAPHY                                                [Edit ✏️]   │
│  ──────────────────────────────────────────────────────────────────────│
│  Family:    Inter, -apple-system, BlinkMacSystemFont, sans-serif       │
│                                                                         │
│  Detected size scale:                                                   │
│  xs: 12px │ sm: 14px │ base: 16px │ lg: 18px │ xl: 20px │ 2xl: 24px   │
│  3xl: 30px │ 4xl: 36px │ 5xl: 48px                                     │
│                                                                         │
│  Detected weights:  400 (normal)  500 (medium)  600 (semibold)  700    │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│  SPACING (Base 4px detected)                               [Edit ✏️]   │
│  ──────────────────────────────────────────────────────────────────────│
│  1: 4px │ 2: 8px │ 3: 12px │ 4: 16px │ 5: 20px │ 6: 24px │ 8: 32px    │
│  10: 40px │ 12: 48px │ 16: 64px │ 20: 80px │ 24: 96px                  │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│  SHADOWS                                                   [Edit ✏️]   │
│  ──────────────────────────────────────────────────────────────────────│
│  sm:  0 1px 2px rgba(0,0,0,0.05)        ░░                             │
│  md:  0 4px 6px rgba(0,0,0,0.07)        ▒▒                             │
│  lg:  0 10px 15px rgba(0,0,0,0.1)       ▓▓                             │
│  xl:  0 20px 25px rgba(0,0,0,0.15)      ██                             │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│  BORDERS                                                   [Edit ✏️]   │
│  ──────────────────────────────────────────────────────────────────────│
│  Radius:   sm: 4px │ md: 8px │ lg: 12px │ xl: 16px │ full: 9999px     │
│  Widths:   1px, 2px                                                    │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│                                                                         │
│  Is everything correct?                                                 │
│                                                                         │
│        [← Back]        [Adjust Values]        [Confirm & Generate →]   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## Step 2b: Screenshot Analysis

### Input

User provides 1-5 images (PNG, JPG, WEBP, max 10MB each).

```
┌─────────────────────────────────────────────────────────────────────────┐
│  SCREENSHOT ANALYSIS                                                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Send reference images:                                                 │
│                                                                         │
│  💡 Tips for best results:                                              │
│  • Include variety (home, form, list views)                            │
│  • Higher resolution = better color analysis                           │
│  • Export Figma mockups in high quality                                │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### Analysis Process

1. **Visual analysis** of each image
2. **Dominant color extraction** from pixels
3. **Font detection** (approximate, based on visual characteristics)
4. **Layout pattern recognition**

### Confirmation Questions

Because screenshot analysis is approximate, ask confirmation questions:

```
┌─────────────────────────────────────────────────────────────────────────┐
│  VISUAL ANALYSIS COMPLETE                                               │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ⚠️ Values extracted visually are APPROXIMATE.                         │
│     Please review carefully and adjust exact values.                    │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│  DETECTED COLORS                                                        │
│  ──────────────────────────────────────────────────────────────────────│
│  I identified these predominant colors:                                 │
│                                                                         │
│  ● Primary (?)    #6366f1  ████  ← Appears to be main color            │
│  ● Secondary (?)  #1e293b  ████  ← Text/dark elements                  │
│  ● Background     #ffffff  ████  ← Main background                     │
│  ● Accent (?)     #22c55e  ████  ← Seen in success buttons             │
│                                                                         │
│  ❓ Confirm: Which is the PRIMARY brand color?                         │
│     ○ #6366f1 (purple)                                                  │
│     ○ #1e293b (dark blue)                                               │
│     ○ Other: [___________]                                              │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│  TYPOGRAPHY                                                             │
│  ──────────────────────────────────────────────────────────────────────│
│  Detected a modern sans-serif font, possibly:                           │
│  • Inter, Roboto, SF Pro, or similar                                   │
│                                                                         │
│  ❓ Which font do you want to use?                                     │
│     ○ Inter (recommended, free)                                         │
│     ○ Roboto                                                            │
│     ○ SF Pro (Apple only)                                               │
│     ○ Other: [___________]                                              │
│                                                                         │
│  ❓ Which size scale to use?                                           │
│     ○ Tailwind (12, 14, 16, 18, 20, 24, 30, 36, 48, 60, 72)            │
│     ○ Material Design (12, 14, 16, 20, 24, 34, 48, 60)                 │
│     ○ Custom: [Define →]                                                │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│  SPACING                                                                │
│  ──────────────────────────────────────────────────────────────────────│
│  Appears to use a consistent spacing system.                            │
│                                                                         │
│  ❓ Which spacing base to use?                                         │
│     ○ 4px (more granular: 4, 8, 12, 16, 20, 24...)                     │
│     ○ 8px (more common: 8, 16, 24, 32, 40, 48...)                      │
│                                                                         │
│  ══════════════════════════════════════════════════════════════════════│
│  SHADOWS AND BORDERS                                                    │
│  ──────────────────────────────────────────────────────────────────────│
│  Observed:                                                              │
│  • Subtle shadows on cards                                              │
│  • Medium border radius (~8px apparent)                                 │
│                                                                         │
│  ❓ Shadow style?                                                       │
│     ○ Subtle (Tailwind-like)                                            │
│     ○ Pronounced (Material-like)                                        │
│     ○ No shadows (flat design)                                          │
│                                                                         │
│  ❓ Border radius?                                                      │
│     ○ Soft (4px, 8px, 12px)                                             │
│     ○ Rounded (8px, 16px, 24px)                                         │
│     ○ Minimal (2px, 4px, 6px)                                           │
│                                                                         │
│        [← Back]                         [Generate Design System →]      │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### Missing Information

If crucial information couldn't be extracted, ask explicitly:

```
❓ I couldn't determine the following. Please provide:

1. Primary brand color (hex): [#________]
2. Font family preference: [___________]
3. Do you want dark mode? ○ Yes  ○ No
```

---

## Step 2c: Manual Questionnaire

12-15 structured questions covering all design tokens.

### Question Categories

| # | Category | Questions |
|---|----------|-----------|
| 1-2 | Colors | Primary, secondary, semantic colors |
| 3-4 | Typography | Font family, code font |
| 5-6 | Typography | Size scale, weights |
| 7-8 | Spacing | Base unit, scale |
| 9-10 | Shadows | Style, intensity |
| 11 | Borders | Radius scale |
| 12 | Dark Mode | Include or not |

### Question Flow

```
┌─────────────────────────────────────────────────────────────────────────┐
│  DESIGN SYSTEM QUESTIONNAIRE                                            │
│     Question 1 of 12                                                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  🎨 BRAND COLORS                                                        │
│                                                                         │
│  What is your PRIMARY brand color?                                      │
│  (Used in main buttons, links, highlight elements)                      │
│                                                                         │
│  [#________] 🎨  or choose a preset:                                    │
│                                                                         │
│  ○ #3b82f6 ████ Blue                                                   │
│  ○ #8b5cf6 ████ Purple                                                 │
│  ○ #22c55e ████ Green                                                  │
│  ○ #f59e0b ████ Orange                                                 │
│  ○ #ef4444 ████ Red                                                    │
│  ○ #06b6d4 ████ Cyan                                                   │
│                                                                         │
│                    [← Previous]    [Next →]                             │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

```
┌─────────────────────────────────────────────────────────────────────────┐
│  DESIGN SYSTEM QUESTIONNAIRE                                            │
│     Question 2 of 12                                                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  🎨 ADDITIONAL COLORS                                                   │
│                                                                         │
│  What SECONDARY color?                                                  │
│  (Support elements, text, alternate backgrounds)                        │
│                                                                         │
│  [#________] 🎨                                                         │
│                                                                         │
│  [ ] Auto-generate complementary palette                                │
│      (based on primary color)                                           │
│                                                                         │
│  ──────────────────────────────────────────────────────────────────────│
│                                                                         │
│  Include standard semantic colors?                                      │
│                                                                         │
│  [✓] Success (green)    #22c55e ████                                   │
│  [✓] Warning (yellow)   #f59e0b ████                                   │
│  [✓] Error (red)        #ef4444 ████                                   │
│  [✓] Info (blue)        #3b82f6 ████                                   │
│                                                                         │
│                    [← Previous]    [Next →]                             │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

```
┌─────────────────────────────────────────────────────────────────────────┐
│  DESIGN SYSTEM QUESTIONNAIRE                                            │
│     Question 5 of 12                                                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  🔤 TYPOGRAPHY                                                          │
│                                                                         │
│  Which main font family?                                                │
│                                                                         │
│  ○ Inter          (modern, clean, great readability)                   │
│  ○ Roboto         (Google, familiar, versatile)                        │
│  ○ Open Sans      (classic, professional)                              │
│  ○ Poppins        (geometric, friendly)                                │
│  ○ Source Sans    (Adobe, technical)                                   │
│  ○ Nunito         (rounded, friendly)                                  │
│  ○ Other: [___________________]                                         │
│                                                                         │
│  ──────────────────────────────────────────────────────────────────────│
│                                                                         │
│  Use a different font for code?                                         │
│                                                                         │
│  ○ Yes → [JetBrains Mono ▼]                                             │
│  ○ No, use the same                                                     │
│                                                                         │
│                    [← Previous]    [Next →]                             │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

```
┌─────────────────────────────────────────────────────────────────────────┐
│  DESIGN SYSTEM QUESTIONNAIRE                                            │
│     Question 12 of 12                                                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  🌙 DARK MODE                                                           │
│                                                                         │
│  Generate tokens for dark mode?                                         │
│                                                                         │
│  ○ Yes, auto-generate (invert palette)                                  │
│  ○ Yes, but I'll define colors manually later                           │
│  ○ No, light mode only                                                  │
│                                                                         │
│  ──────────────────────────────────────────────────────────────────────│
│                                                                         │
│  ✅ Questionnaire complete!                                             │
│                                                                         │
│                    [← Previous]    [Generate Design System →]           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## Step 3: Generate Files

After confirmation, generate the design system files.

### Files Generated

1. **`design/tokens.json`** - DTCG format (see dtcg-spec.md)
2. **`design/{stack-config}`** - Stack-specific configuration:
   - Tailwind: `tailwind.config.ts`
   - CSS vanilla: `variables.css`
   - SASS: `_variables.scss`
   - CSS-in-JS: `theme.ts`
3. **`design/README.md`** - Human-readable documentation

### Completion Screen

```
┌─────────────────────────────────────────────────────────────────────────┐
│  ✅ DESIGN SYSTEM GENERATED                                            │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Your DTCG file was created successfully!                              │
│                                                                         │
│  📄 design/tokens.json                                                  │
│  ├── 12 colors (4 brand + 4 semantic + 4 neutral)                       │
│  ├── 9 font sizes                                                       │
│  ├── 4 font weights                                                     │
│  ├── 12 spacing values                                                  │
│  ├── 4 shadows                                                          │
│  └── 5 border-radius values                                             │
│                                                                         │
│  📄 design/tailwind.config.ts                                           │
│  └── Theme extension with all tokens mapped                             │
│                                                                         │
│  📄 design/README.md                                                    │
│  └── Human-readable documentation                                       │
│                                                                         │
│  ──────────────────────────────────────────────────────────────────────│
│                                                                         │
│  These tokens will be used automatically when generating UI             │
│  components via the frontend-design skill.                              │
│                                                                         │
│                                    [Continue to Project Setup →]        │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## Integration with frontend-design Skill

When the `wxcode-executor` detects a UI generation task:

1. **Check for design tokens:**
   ```bash
   [ -f design/tokens.json ] && echo "DESIGN SYSTEM FOUND"
   ```

2. **Read tokens and include as context:**
   ```
   /frontend-design

   ## Design System (Source of Truth)

   Use these design tokens for all visual decisions.
   Do NOT deviate from these values unless explicitly requested.

   ```json
   {content of design/tokens.json}
   ```

   ## Task

   {component/page description from the plan}
   ```

3. **The skill generates code using the tokens**

---

## Error Handling

### URL Extraction Failures

| Error | Fallback |
|-------|----------|
| WebFetch blocked (CSP, 403) | Try MCP Playwright |
| Playwright fails | Offer Screenshot or Questionnaire |
| Site requires auth | Ask for screenshot instead |
| No CSS found | Ask for screenshot instead |

### Screenshot Analysis Limitations

| Limitation | Mitigation |
|------------|------------|
| Color calibration variance | Confirmation questions |
| Font detection imprecise | Offer common font presets |
| Exact pixel values unknown | Use standard scales |

---

## References

- **DTCG Spec:** See `.wxcode/conversion/dtcg-spec.md`
- **Template:** See `wxcode-skill/templates/design-tokens.json`
- **frontend-design Skill:** Claude Code built-in skill for UI generation
