---
name: wxcode:quick
description: Execute a quick task with WXCODE guarantees (atomic commits, state tracking) but skip optional agents
argument-hint: ""
allowed-tools:
  - Read
  - Write
...
<objective>
Execute small, ad-hoc tasks with WXCODE guarantees (atomic commits, STATE.md tracking) while skipping optional agents (research, plan-checker, verifier).

Quick mode is the same system with a shorter path:
- Spawns wxcode-planner (quick mode) + wxcode-executor(s)
- Skips wxcode-phase-researcher, wxcode-plan-checker, wxcode-verifier
...
Check that an active WXCODE project exists:

```bash
if [ ! -f .planning/ROADMAP.md ]; then
  echo "Quick mode requires an active project with ROADMAP.md."
  echo "Run /wxcode:new-project first."
  exit 1
fi
```
...
Spawn wxcode-planner with quick mode context:
...
Spawn wxcode-executor with plan reference:
...
Ready for next task: /wxcode:quick
```
