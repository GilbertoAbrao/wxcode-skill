...
Todos are captured during work sessions with /wxcode:add-todo.

---

Would you like to:

1. Continue with current phase (/wxcode:progress)
2. Add a todo now (/wxcode:add-todo)
```
...
- `/wxcode:check-todos` → show all
- `/wxcode:check-todos api` → filter to area:api only
</step>
...
- `/wxcode:check-todos [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /wxcode:add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/wxcode:add-phase [description from todo]`
...
- Don't create plans from this command — route to /wxcode:plan-phase or /wxcode:add-phase
</anti_patterns>
