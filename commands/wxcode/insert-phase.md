...
Example: `/wxcode:insert-phase 72 Fix critical auth bug`
→ after = 72
...
if [ $# -lt 2 ]; then
  ...
  echo "Usage: /wxcode:insert-phase <after> <description>"
  echo "Example: /wxcode:insert-phase 72 Fix critical auth bug"
  exit 1
fi
```
...
2. Insert new phase heading with (INSERTED) marker:
   ...
   **Plans:** 0 plans
   ...
   Plans:
   - [ ] TBD (run /wxcode:plan-phase {decimal_phase} to break down)
...
- Don't use this for planned work at end of milestone (use /wxcode:add-phase)
